...
function mountLetsScroll(container, config) {
  const reduce = false; // Forced false to ensure frames always play regardless of OS settings
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, framePath: s.framePath, frameCount: s.frameCount, still: s.still, stillM: s.stillMobile,
                   accent: s.accent, w: s.scroll || DIVE_W, linger: s.linger || 0 };
    SEGMENTS.push(dive);
    s._seg = dive;
    if (i < N - 1 && CONNECTORS[i]) {
      SEGMENTS.push({ kind: 'conn', si: i, framePath: CONNECTORS[i].framePath, frameCount: CONNECTORS[i].frameCount,
  ...
  SEGMENTS.forEach(s => {
    ...
    s.el = scene; s.img = img; s.canvas = null; s.ctx = null; s.hasClip = false;
    s.loading = false; s.ready = false; s.cur = 0; s.target = 0; s.visible = false;
    s.lastFrame = -1;
  });

  // Advanced Frame Cache
  ...
  async function fetchFrame(url) {
    ...
    frameCache.set(url, { promise, lastUsed: Date.now() });

    // LRU Pruning
    if (frameCache.size > CACHE_SIZE) {
      let oldest = Date.now();
      let oldestKey = null;
      for (const [k, v] of frameCache.entries()) {
  ...
  function loadClip(s) {
    if (reduce || s.loading || !s.framePath || !s.frameCount) return;
    s.loading = true;
    
    const canvas = document.createElement('canvas');
    canvas.className = 'sw-scene__canvas'; // reusing the CSS class name so layout stays identical
    canvas.width = 1280;
    canvas.height = 720;
    s.ctx = canvas.getContext('2d', { alpha: false });
    
    // Kick off the first frame
    const firstUrl = s.framePath.replace('$$', pad4(1));
    fetchFrame(firstUrl).then(img => {
      if (img) {
        s.ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        if (!s.ready) {
    ...
    // Preload the first 10 frames of this scene immediately so the transition is seamless
    for (let p = 2; p <= Math.min(10, s.frameCount); p++) {
       fetchFrame(s.framePath.replace('$$', pad4(p)));
    }
  }
  ...
  function raf() {
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (!s.hasClip || !s.canvas) continue;
      if (!s.visible && Math.abs(s.cur - s.target) < 0.002) continue;
      s.cur += (s.target - s.cur) * (reduce ? 1 : 0.06);
      
      const dur = s.frameCount - 1;
      let t = clamp(Math.floor(s.cur * dur), 0, dur);
      
      // Device detection logic: Step by 2 on mobile to reduce decoding overhead by half!
      if (isMobile()) {
         t = Math.floor(t / 2) * 2;
      }
      
      if (s.lastFrame !== t) {
        s.lastFrame = t; // Optimistically set to prevent refetching
        
        const url = s.framePath.replace('$$', pad4(t + 1));
        fetchFrame(url).then(img => {
          if (img && s.lastFrame === t) {
             // Draw directly to hardware accelerated canvas, avoiding DOM reflows
             s.ctx.drawImage(img, 0, 0, s.canvas.width, s.canvas.height);
          }
        });
        
        // Silently fetch the next few frames into browser cache so they are ready
        const lookaheadCount = isMobile() ? 4 : 8; // less aggressive on mobile
        for (let lookahead = 1; lookahead <= lookaheadCount; lookahead++) {
          const nextT = t + (isMobile() ? lookahead * 2 : lookahead);
          if (nextT < s.frameCount) {
             fetchFrame(s.framePath.replace('$$', pad4(nextT + 1)));
          }
        }
      }
    }
...
function injectCSS() {
  ...
  .sw-scene__canvas,.sw-scene__still{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 42%;}
  .sw-scene__still{will-change:transform;} .sw-scene.has-clip .sw-scene__still{opacity:0;} .sw-scene__canvas{z-index:1;}
  .sw-copylayer{position:fixed;inset:0;z-index:20;pointer-events:none;}
  .sw-copylayer::before{content:"";position:absolute;inset:0;width:min(58vw,780px);background:transparent;}
  .sw-copy{
    ...
    background:color-mix(in srgb, var(--sw-bg) 65%, transparent);
    backdrop-filter:blur(24px) saturate(180%);
    -webkit-backdrop-filter:blur(24px) saturate(180%);
    border:1px solid color-mix(in srgb, var(--sw-ink) 8%, transparent);
    padding:clamp(28px, 4vw, 48px);
    border-radius:24px;
    box-shadow:0 30px 60px rgba(0,0,0,0.12), 0 0 0 1px inset rgba(255,255,255,0.4);
  }
  ...
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:600;font-style:italic;color:var(--sw-ink);font-size:clamp(2.4rem,4.4vw,3.8rem);line-height:1.03;margin:12px 0 0;letter-spacing:-.02em;}
  .sw-copy__body{margin-top:20px;font-size:clamp(1.05rem,1.25vw,1.15rem);line-height:1.6;color:color-mix(in srgb,var(--sw-ink) 78%,var(--sw-ink-soft));max-width:40ch;}
  ...
  .sw-btn{text-decoration:none;font-weight:600;font-size:.95rem;padding:14px 28px;border-radius:999px;transition:all .3s cubic-bezier(0.2, 0.8, 0.2, 1); letter-spacing:0.02em;}
  .sw-btn--primary{color:#fff;background:var(--sw-ink);box-shadow:0 8px 24px color-mix(in srgb,var(--sw-ink) 30%,transparent);} 
  .sw-btn--primary:hover{transform:translateY(-3px);box-shadow:0 12px 32px color-mix(in srgb,var(--sw-ink) 40%,transparent); background:color-mix(in srgb,var(--sw-ink) 85%, #000);}
  .sw-btn--ghost{color:var(--sw-ink);border:1.5px solid color-mix(in srgb,var(--sw-ink) 20%,transparent); background:transparent;} 
  .sw-btn--ghost:hover{transform:translateY(-3px); border-color:var(--sw-ink); background:color-mix(in srgb,var(--sw-ink) 4%,transparent);}
  ...
  @media (max-width:860px){
    .sw-nav{display:none;}
    .sw-copylayer::before{width:100%;height:60%;top:auto;bottom:0;background:transparent;}
    ...
    .sw-copy{left:clamp(16px,4vw,32px);right:clamp(16px,4vw,32px);top:auto;bottom:clamp(64px,14vh,120px);transform:none;width:auto;max-width:560px;padding:24px;border-radius:20px;}
    .sw-copy{bottom:calc(clamp(56px,12dvh,110px) + env(safe-area-inset-bottom));}
    .sw-copy__title{font-size:clamp(2.2rem,8.5vw,2.9rem);}
